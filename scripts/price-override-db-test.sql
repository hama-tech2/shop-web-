...
declare
  ...
  v_amount   numeric;
  v_list     numeric;
  -- Deliberately not the list price: if the two were equal, every check
  -- below would pass whether the override applied or not.
  v_over     numeric := 777;
begin
  ...
  -- Read, not pinned: the six-month list price is TEMPORARILY 1,000 for
  -- one live payment test. What this file is about is the override, and
  -- that has to hold at whatever the list price happens to be.
  v_list := app.plan_price('months_6');
  if v_list is null or v_list <= 0 then
    raise exception 'FAIL six months has no list price';
  end if;
  if app.plan_price('year_1') <> 72000 then
    raise exception 'FAIL the list price of a year moved: %', app.plan_price('year_1');
  end if;
  raise notice 'PASS the list price is % for six months and 72,000 for a year', v_list;

  -- ---------- 2. no override: everybody pays the list ----------
  if app.plan_price_for('months_6', v_shop) <> v_list then
  ...
  insert into app.price_overrides(shop_id, plan, amount, expires_at, note)
       values (v_shop, 'months_6', v_over, now() + interval '1 hour', 'live payment test');

  if app.plan_price_for('months_6', v_shop) <> v_over then
    raise exception 'FAIL the override shop pays % for six months', app.plan_price_for('months_6', v_shop);
  end if;
  raise notice 'PASS the named shop pays % for six months', v_over;
  ...
  if app.plan_price_for('months_6', v_shop2) <> v_list then
  ...
  v_intent := v_row.id;
  if v_row.amount <> v_over then
    raise exception 'FAIL the checkout was created at %, not %', v_row.amount, v_over;
  end if;
  raise notice 'PASS the intent the server writes is the override — what Wayl will be asked for';
  ...
  begin
    perform public.wayl_apply_payment(v_intent, v_ref, v_over - 1, 'Wallet', 'wayl');
  ...
  begin
    perform public.wayl_apply_payment(v_intent, v_ref, v_list + 1, 'Wallet', 'wayl');
  ...
  select * into v_row from public.wayl_apply_payment(v_intent, v_ref, v_over, 'Wallet', 'wayl');
  if not v_row.activated then raise exception 'FAIL the override amount did not activate'; end if;
  raise notice 'PASS the override amount activates, and nothing else does';
  ...
  if v_amount <> v_over then
    raise exception 'FAIL the payment was recorded as %, not %', v_amount, v_over;
  end if;
  ...
  raise notice 'PASS exactly six months granted, and the override recorded as what was paid';
