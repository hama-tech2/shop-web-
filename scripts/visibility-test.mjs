 ...
 *
 * 2. A product is shown everywhere (Home + search + profile + link) or
 *    kept off Home only (search + profile + link). Profile-only is NOT
 *    hidden: the product stays public, searchable and indexable, and it
 *    still counts toward the Free plan's five.
 *
...
check('default product is in search', await inSearch(), true);
...
check('profile-only product is STILL in search', await inSearch(), true);
...
check('profile-only product remains in the public sitemap',
  sitemap.includes(PRODUCT_ID), true);
...
check('the create form has one marketplace category field',
  (createForm.match(/name="category"/g) || []).length, 1);
check('the marketplace category label is simplified to پۆل',
  createForm.includes('>پۆل <span class="field__optional">'), true);
check('the category is visibly optional', createForm.includes('ئارەزوومەندانە'), true);
check('the create form has no shop-category selector',
  createForm.includes('name="own_category"') || createForm.includes('id="f-own-category"'), false);
check('the create form has no inline shop-category creation',
  createForm.includes('category-add-open') || createForm.includes('+ بەشی نوێ'), false);
check('the old marketplace category label is gone', createForm.includes('پۆلی بازاڕ'), false);
