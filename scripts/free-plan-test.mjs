...
for (const held of [0, 1, 2, 3, 4]) {
  ...
  check(`product ${held + 1} publishes`, r.location, '/app/products');
}
...
check('deleting one gives the slot back',
  (await page('/app/new?plan=free')).includes('publish-page'));
check('and the next product publishes again', (await publish(1)).location, '/app/products');
...
await free(0);
check(`${FREE_IMAGE_LIMIT} image publishes on Free`, (await publish(1)).location, '/app/products');
...
check('who can still publish', (await publish(1)).location, '/app/products');
check('with more than one image', (await publish(5)).location, '/app/products');
