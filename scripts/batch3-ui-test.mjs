...
try {
  ...
  await fetch(STUB + '/__plan/year_1'); await fetch(STUB + '/__sub/20');
  {
    ...
    await page.goto(APP + '/app?e=errGone');
    check('owner home: reachable with the delete error, and no old manager list', await page.locator('.alert').count() > 0 && await page.locator('.manager-filters').count() === 0);
    await page.goto(APP + '/app/products');
    check('the retired manager address lands on the owner home', new URL(page.url()).pathname === '/app');
