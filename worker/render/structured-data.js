 ...
 */
export function shopLd({ shop, origin, productCount }) {
  ...
  if (Number.isFinite(productCount) && productCount > 0) {
    ld.makesOffer = { '@type': 'Offer', itemOffered: { '@type': 'Product' } };
  }
  return ld;
}
 ...
 * together exactly as the page reads them. availability is InStock and
 * nothing finer: the product is live on a public shop, which is the
 * whole of what this app knows about whether it can be bought — there
 * is no stock count to report and none is claimed.
 *
...
export function productLd({ product, shop }) {
  ...
  if (shop.name) ld.brand = { '@type': 'Brand', name: shop.name };
  return ld;
}
